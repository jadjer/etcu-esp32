 ...
 */
constexpr uint32_t
calculateScaledValue(uint32_t
const value,
uint32_t const scalingBase,
float const scalingFactor
)
{
return scalingBase + static_cast<uint32_t>(static_cast
<float>(value)
* scalingFactor);
}

Scaller::Scaller(uint32_t const minimalValue, uint32_t const maximalValue) :
...
void Scaller::registerChangeValueCallback(ChangeValueCallbackFunction const &changeValueCallbackFunction) {
  m_changeValueCallbackFunction = changeValueCallbackFunction;
}

void Scaller::setValue(uint32_t const value) {
