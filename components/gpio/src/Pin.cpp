...
Pin::Pin(uint8_t const numberOfPin, bool const reverseValue) : m_reverseValue(reverseValue), m_numberOfPin(numberOfPin)
{
...
PinLevel Pin::getLevel() const
{
    ...
    if (m_reverseValue)
    {
        auto const gpioLevelReversed = not gpioLevel;

        pinLevel = static_cast<PinLevel>(gpioLevelReversed);
    }
